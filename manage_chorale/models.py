    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["user", "chorale"], name="uniq_user_chorale"
            ),
        ]
        indexes = [
...
class Contribution(models.Model):
    ...
    chorale = models.ForeignKey(
        Chorale, on_delete=models.CASCADE, related_name="contributions", db_index=True
    )
    title = models.CharField(max_length=100)
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text=_("Amount expected per member")
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["chorale", "title"], name="uniq_contrib_per_chorale"
            ),
        ]
...
class MemberContribution(models.Model):
    ...
    contribution = models.ForeignKey(
        Contribution, on_delete=models.CASCADE, related_name="payments"
    )
...
class CashFlow(models.Model):
    ...
    chorale = models.ForeignKey(
        Chorale, on_delete=models.CASCADE, related_name="cash_flows", db_index=True
    )
    title = models.CharField(max_length=100)
    type_cash_flow = models.CharField(
        max_length=20, choices=TYPE_CHOICES, default=TYPE_ENTREE
    )
...
class ChoraleEvent(models.Model):
    ...
    event_type = models.CharField(
        max_length=20, choices=EVENT_TYPE_CHOICES, default="practice"
    )
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="created_chorale_events",
    )
...
class Absence(models.Model):
    ...
    event = models.ForeignKey(
        "ChoraleEvent", on_delete=models.CASCADE, related_name="absences", db_index=True
    )
    member = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["event", "member"], name="uniq_absence_per_event_member"
            ),
        ]
...
class Sanction(models.Model):
    ...
    chorale = models.ForeignKey(
        Chorale, on_delete=models.CASCADE, related_name="sanctions", db_index=True
    )
    member = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="sanctions"
    )
    sanction_type = models.CharField(
        max_length=20, choices=SANCTION_TYPE_CHOICES, default=SANCTION_WARNING
    )
    ...
    lifted_at = models.DateField(
        null=True, blank=True, help_text=_("Lift date — null = sanction active")
    )
    recorded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
...
class Event(models.Model):
    ...
    chorale = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="events",
        help_text=_("Chorale concerned by the event"),
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
    ...
    timestamp = models.DateTimeField(
        default=timezone.now, db_index=True, help_text=_("Date and time of the event")
    )
    ...
    event_type = models.CharField(
        max_length=50, choices=EVENT_TYPES, help_text=_("Event type")
    )
    description = models.TextField(help_text=_("Detailed description of the event"))
    short_description = models.CharField(
        max_length=255, blank=True, help_text=_("Short description for notifications")
    )
    comment = models.TextField(
        blank=True, help_text=_("Additional comment (e.g., reason for a sanction)")
    )
    content_type = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Type of the related object (e.g., Contribution, Sanction)"),
    )
    ...
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Contextual data (old value, new value, etc.)"),
    )
    ip_address = models.GenericIPAddressField(
        null=True,
        blank=True,
        help_text=_("User's IP address at the time of the action"),
    )
    user_agent = models.TextField(
        blank=True, help_text=_("User agent of the browser/device")
    )
    ...
    def __str__(self):
        return (
            f"[{self.chorale.name}] {self.get_event_type_display()} - {self.timestamp}"
        )
